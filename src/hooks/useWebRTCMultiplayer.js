...
async function storeData(obj) {
  ...
  if (!text)
    throw new Error(`Worker returned empty response (status ${res.status})`);
  try {
...
export function useWebRTCMultiplayer({ onStateReceived, onPeerMessage }) {
  ...
  const createOfferForSlot = useCallback(
    async (slotId) => {
      try {
  ...
  const receiveAnswer = useCallback(async (answerCode) => {
    try {
      ...
      if (!peer) {
        setError(
          "No pending offer for this answer. Generate an offer for this slot first.",
        );
        return false;
      }
