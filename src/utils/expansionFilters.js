...
export function filterFactionsByExpansions(factions, expansionsEnabled = {}) {
  ...
  if (!expansionsEnabled.pok)
    out = out.filter((f) => !pokExclusions.factions.includes(f.name));
...
export function filterTilesByExpansions(tiles, expansionsEnabled = {}) {
  let out = [...tiles];
  if (!expansionsEnabled.pok)
    out = out.filter((t) => !pokExclusions.tiles.includes(t.id));
  if (!expansionsEnabled.te)
    out = out.filter((t) => !teExclusions.tiles.includes(t.id));
  return out;
}
