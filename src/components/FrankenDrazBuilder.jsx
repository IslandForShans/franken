...
export default function FrankenDrazBuilder({
  ...
  const isComponentAdded = (componentName, factionName) => {
    return categories.some((cat) => {
      ...
      return items.some(
        (item) => item.name === componentName && item.faction === factionName,
      );
    });
  };
  ...
  const renderComponentCard = (component, category) => {
    ...
    const isTile = ["blue_tiles", "red_tiles", "home_systems"].includes(
      category,
    );
    const isBreakthrough = category === "breakthrough";
    const typeIcon = component.tech_type
      ? TECH_ICONS[component.tech_type.toLowerCase()]
      : null;
          ...
          <div className="flex-1 min-w-0">
            ...
            {isUnit && component.combat && (
              ...
              >
                {component.cost !== undefined && (
                  <span className="font-semibold">Cost: {component.cost}</span>
                )}
                <span className="font-semibold">
                  Combat: {component.combat}
                </span>
                {component.move !== undefined && (
                  <span className="font-semibold">Move: {component.move}</span>
                )}
                ...
                <div className="text-xs mt-1" style={{ color: "#fbbf24" }}>
                  ...
                  <span className="ml-1 inline-flex flex-wrap items-center gap-2">
                    {component.synergy.map((s, idx) => {
                      ...
                      const secondaryIcon =
                        TECH_ICONS[s.secondary?.toLowerCase()];

                      if (!primaryIcon || !secondaryIcon) {
                        return (
                          <span key={idx}>{`${s.primary}/${s.secondary}`}</span>
                        );
                      }
                      ...
                      return (
                        ...
                        >
                          ...
                          <img
                            src={secondaryIcon}
                            alt={`${s.secondary} tech`}
                            title={`${s.secondary} tech`}
                            className="w-4 h-4"
                          />
                        </span>
                      );
            ...
            {isTile && component.planets && component.planets.length > 0 && (
              <div className="text-xs mt-1" style={{ color: "#6ee7b7" }}>
                <span className="font-semibold">
                  {component.planets.length} Planet
                  {component.planets.length !== 1 ? "s" : ""}:
                </span>
                {component.planets.map((p, idx) => (
                  ...
                  >
                    ...
                    {p.legendary_ability && (
                      <div style={{ color: "#fcd34d", fontStyle: "italic" }}>
            ...
            {isTile && component.wormhole && (
              <div className="text-xs mt-1" style={{ color: "#c084fc" }}>
                <span className="font-semibold">Wormhole:</span>{" "}
                {component.wormhole}
              </div>
            )}
            {isTile &&
        ...
        <div className="flex flex-wrap gap-2">
          {(draftedItems.factions || []).map((faction, idx) => (
            <div
              ...
              onClick={() =>
                setExpandedFaction(
                  expandedFaction === faction.name ? null : faction.name,
                )
              }
            >
              {faction.icon && (
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 bg-blue-900/20 rounded border border-blue-600">
          <h4 className="font-semibold text-blue-400 mb-2 text-sm">
            Blue Tiles ({(draftedItems.blue_tiles || []).length})
          </h4>
          <div className="space-y-1">
            ...
            {(draftedItems.blue_tiles || []).length > 3 && (
              <div className="text-xs text-gray-400">
                +{(draftedItems.blue_tiles || []).length - 3} more
              </div>
            )}
          </div>
        </div>
        <div className="p-3 bg-red-900/20 rounded border border-red-600">
          <h4 className="font-semibold text-red-400 mb-2 text-sm">
            Red Tiles ({(draftedItems.red_tiles || []).length})
          </h4>
          <div className="space-y-1">
            ...
            {(draftedItems.red_tiles || []).length > 3 && (
              <div className="text-xs text-gray-400">
                +{(draftedItems.red_tiles || []).length - 3} more
              </div>
            )}
          </div>
        </div>
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            ...
            {getAvailableComponents(selectedCategory).length === 0 ? (
              <div className="text-sm text-gray-400 p-3 bg-gray-900/50 rounded">
                No {formatCategoryName(selectedCategory).toLowerCase()}{" "}
                available from your drafted factions
              </div>
            ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-2">
                    {items.map((item, idx) =>
                      renderBuiltComponent(item, cat, idx),
                    )}
                  </div>
                </div>
              );
