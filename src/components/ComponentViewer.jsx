...
export default function ComponentViewer({
  ...
  const picksRemaining = limitForCategory
    ? Math.max(0, limitForCategory - picksSoFar)
    : undefined;
    ...
    return data
      .filter((item) => {
        ...
        const matchesAnomalies =
          item.anomalies?.some((anomaly) =>
            anomaly.toLowerCase().includes(q),
          ) || false;
        const matchesWormhole =
          item.wormhole?.toLowerCase().includes(q) || false;
  ...
  const handleDragStart = (e, item) => {
    ...
    e.dataTransfer.setData(
      "application/json",
      JSON.stringify({ category, componentId: id }),
    );
            ...
            >
              <div className="flex justify-between items-start">
                <div style={{ flex: 1 }}>
                  ...
                  {item.faction && (
                    <div className="component-faction">{item.faction}</div>
                  )}
                      ...
                      {item.planets?.map((p, idx) => (
                        <div key={p.name + idx} className="planet-item">
                          ...
                          <div className="planet-resources">
                            <span className="planet-resource-value">
                              Resources: {p.resource || 0}
                            </span>
                            <span>•</span>
                            <span className="planet-influence-value">
              ...
              {isDisabled && (
                <div className="component-limit-overlay">
                  <div className="component-limit-badge">Limit reached</div>
                </div>
              )}
            </div>
          );
