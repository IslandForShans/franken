...
export default function DraftSimulator({
  ...
  const [sidebarCollapseAllTrigger, setSidebarCollapseAllTrigger] =
    useState(null);
  ...
  useEffect(() => {
    ...
    const draftBaseLimits =
      draftVariant === "power" ? powerDraftLimits : defaultDraftLimits;
  ...
  const getFilteredComponents = useCallback(
    (category, effectivePlayerCount = playerCount) => {
      ...
      const factionComponents = expansionsEnabled.dsOnly
        ? []
        : factionsData.factions
            .filter((f) => !bannedFactions.has(f.name))
            .filter(
              (f) =>
                expansionsEnabled.pok ||
                !pokExclusions.factions.includes(f.name),
            )
            .filter(
              (f) =>
                expansionsEnabled.te || !teExclusions.factions.includes(f.name),
            )
            .filter(
              (f) =>
                expansionsEnabled.firmobs ||
                !noFirmament.factions.includes(f.name),
            )
            .filter(
              (f) =>
      ...
      console.log(
        `Total components for ${category}: ${totalComponents.length}`,
      );
  ...
  const createBagsWithUniqueDistribution = useCallback(
    (effectivePlayerCount = playerCount) => {
      const bags = Array.from({ length: effectivePlayerCount }, () => ({}));

      console.log(
        `Creating ${effectivePlayerCount} bags for ${effectivePlayerCount} players`,
      );

      categories.forEach((category) => {
        const allComponents = getFilteredComponents(
          category,
        ...
        if (category === "table_position") {
          const validPositions = allComponents
            .filter(
              (position) =>
                position.position >= 1 &&
                position.position <= effectivePlayerCount,
            )
          ...
          for (
            let playerIdx = 0;
            playerIdx < effectivePlayerCount;
            playerIdx++
          ) {
        ...
        for (let playerIdx = 0; playerIdx < effectivePlayerCount; playerIdx++) {
          ...
          for (
            let i = 0;
            i < bagSize && componentIndex < distributionPool.length;
            i++
          ) {
  ...
  const createBagsForFrankenDraz = useCallback(
    (effectivePlayerCount = playerCount) => {
      ...
      // Get all available factions (names and icons only)
      let allFactions = expansionsEnabled.dsOnly
        ? []
        : factionsData.factions
            .filter((f) => !bannedFactions.has(f.name))
            .filter(
      ...
      for (let playerIdx = 0; playerIdx < effectivePlayerCount; playerIdx++) {
        for (
          let i = 0;
          i < factionsPerBag && factionIndex < factionPool.length;
          i++
        ) {
      ...
      while (bluePool.length < totalBlueNeeded) {
        bluePool = [
          ...
          ...shuffleArray(
            [...blueTiles].map((t) => ({
              ...t,
              copyIndex: bluePool.length / blueTiles.length,
            })),
          ),
        ];
      }
      ...
      for (let playerIdx = 0; playerIdx < effectivePlayerCount; playerIdx++) {
        for (
          let i = 0;
          i < blueTilesPerBag && blueIndex < bluePool.length;
          i++
        ) {
      ...
      while (redPool.length < totalRedNeeded) {
        redPool = [
          ...
          ...shuffleArray(
            [...redTiles].map((t) => ({
              ...t,
              copyIndex: redPool.length / redTiles.length,
            })),
          ),
        ];
      }
  ...
  const initializeDraft = useCallback(
    (settings) => {
      ...
      const emptyFactions = Array.from(
        { length: effectivePlayerCount },
        (_, i) => {
      ...
      setPlayerProgress(
        Array.from({ length: effectivePlayerCount }, () => {
          const p = {};
          categories.forEach((cat) => {
            p[cat] = 0;
          });
    ...
    [
      categories,
      getFilteredComponents,
      createBagsWithUniqueDistribution,
      createBagsForFrankenDraz,
    ],
  );

  const checkAndAdvanceIfNeeded = (
    currentFactions,
    currentProgress,
    playerIdx,
  ) => {
    ...
    if (draftVariant === "frankendraz") {
      ...
      canPick =
        bag.factions?.length > 0 ||
        bag.blue_tiles?.length > 0 ||
        bag.red_tiles?.length > 0;
    } else {
      categories.forEach((cat) => {
        const inBag = bag[cat]?.length || 0;
        const alreadyPicked = progress[cat] || 0;
    ...
    if (!canPick) {
      console.log(
        `Player ${playerIdx + 1} auto-passing on initialization - nothing to draft`,
      );
  ...
  const getMaxPicksForRound = useCallback(() => {
    ...
    if (draftVariant === "frankendraz") {
      ...
      const availableCategories = [
        ...
        "blue_tiles",
        "red_tiles",
      ].filter((cat) => currentBag[cat] && currentBag[cat].length > 0).length;
    ...
    (category, component) => {
      ...
      if (draftVariant === "frankendraz") {
        ...
        const alreadyPicked = pendingPicks.some(
          (p) =>
            p.category === category &&
            (p.component.id || p.component.name) === componentId,
        );
            ...
            prev.filter(
              (pick) =>
                !(
                  ...
                  (pick.component.id || pick.component.name) === componentId
                ),
            ),
          );
          return;
        ...
        const alreadyPickedInCategory = pendingPicks.some(
          (p) => p.category === category,
        );
        if (alreadyPickedInCategory) {
          alert(
            `You can only pick one ${category.replace("_", " ")} per round.`,
          );
          return;
        }
        ...
        if (pendingPicks.length >= maxPicks) {
          alert(
            `You can only pick ${maxPicks} item${maxPicks !== 1 ? "s" : ""} this round.`,
          );
          return;
        }
      ...
      const pendingCountInCategory = pendingPicks.filter(
        (p) => p.category === category,
      ).length;

      if (currentProgress + pendingCountInCategory >= draftLimits[category]) {
        alert(
          `Cannot pick more ${category.replace("_", " ")}. Limit: ${draftLimits[category]}`,
        );
        return;
      }
      ...
      if (pendingPicks.length >= maxPicks) {
        alert(
          `You can only pick ${maxPicks} component${maxPicks !== 1 ? "s" : ""} this round.`,
        );
        return;
      }
  ...
  const handleSubmitPicks = () => {
    ...
    if (isMultiplayerHost) {
      ...
      if (draftVariant === "rotisserie") {
        if (pendingPicks.length !== 1) {
          alert("You must pick exactly 1 component.");
          return;
        }
      } else {
        if (pendingPicks.length < maxPicks) {
          alert(
            `You must pick ${maxPicks} component${maxPicks !== 1 ? "s" : ""} this round.`,
          );
          return;
        }
      }
    ...
    if (draftVariant === "rotisserie") {
      ...
      if (pendingPicks.length < maxPicks) {
        alert(
          `You must pick ${maxPicks} component${maxPicks !== 1 ? "s" : ""} this round. Currently: ${pendingPicks.length}`,
        );
        return;
      }
    ...
    pendingPicks.forEach(({ category, component }) => {
      ...
      if (draftVariant === "rotisserie") {
        setRotisseriePool((prev) => {
          ...
          pool[category] = pool[category].filter(
            (c) => (c.id || c.name) !== compId,
          );
          return pool;
        });
      } else {
        ...
        const compId = component.id || component.name;
        updatedBags[currentBagIdx][category] = updatedBags[currentBagIdx][
          category
        ].filter((c) => (c.id || c.name) !== compId);
      }
    ...
    if (draftVariant === "frankendraz") {
      const allBagsEmpty = updatedBags.every(
        (bag) =>
  ...
  const advanceToNextPlayer = (updatedFactions, updatedProgress) => {
    setTimeout(() => {
      ...
      while (attempts < maxAttempts) {
        ...
        if (draftVariant !== "rotisserie") {
          const nextBag =
            completedRound && playerBags.length > 1
          ...
          if (draftVariant === "frankendraz") {
            ...
            canPick =
              nextBag?.factions?.length > 0 ||
              nextBag?.blue_tiles?.length > 0 ||
              nextBag?.red_tiles?.length > 0;
          } else {
            categories.forEach((cat) => {
          ...
          if (!canPick) {
            console.log(
              `Player ${nextPlayer + 1} auto-passing - nothing they can draft from their bag`,
            );
          ...
          if (allPlayersComplete) {
            ...
            const needsReduction = updatedFactions.some((faction, idx) => {
              return categories.some((cat) => {
                ...
                if (isOver) {
                  console.log(
                    `Player ${idx + 1} ${cat}: ${current}/${limit} - needs reduction`,
                  );
                }
                return isOver;
              });
            ...
            if (needsReduction) {
              ...
              console.log(
                "No reduction needed - adding extra components and completing",
              );
      ...
      categories.forEach((category) => {
        ...
        components.forEach((component) => {
          ...
          if (extraComponents.length > 0) {
            console.log(
              `Adding extra components for ${component.name}:`,
            ...
            extraComponents.forEach((extra) => {
              ...
              const fullComponentData = findFullComponentData(
                extra.name,
                extra.faction || component.faction,
                targetCategory,
              );
  ...
  const getCurrentFactionLimits = useCallback(() => {
    ...
    draftVariant,
    frankenDrazSettings.blueTilesPerBag,
  ...
  const mpCheckAndApplyAllPicks = (incomingPlayerIndex, incomingPicks) => {
    ...
    Object.entries(collected).forEach(([piStr, picks]) => {
      ...
      picks.forEach(({ category, component }) => {
        newFactions[pi][category] = [
          ...(newFactions[pi][category] || []),
          component,
        ];
        const compId = component.id || component.name;
        newBags[pi] = { ...newBags[pi] };
        newBags[pi][category] = (newBags[pi][category] || []).filter(
          (c) => (c.id || c.name) !== compId,
        );
        newHistory.push({
          playerIndex: pi,
    ...
    const allLimitsMet = newFactions.every((faction) =>
      categories.every(
        (cat) => (faction[cat]?.length || 0) >= (draftLimits[cat] || 0),
      ),
    );
    ...
    if (allLimitsMet) {
      ...
      const needsReduction = newFactions.some((faction) =>
        categories.some(
          (cat) =>
            (faction[cat]?.length || 0) > (factionLimits[cat] ?? Infinity),
        ),
      );
  ...
  const mpCheckAndApplyAllFactions = (phase) => {
    ...
    const merged = factions.map((f, i) =>
      collected[i] ? { ...collected[i] } : { ...f },
    );
    const factionLimits = getCurrentFactionLimits();
    ...
    if (phase === "reduction") {
      ...
      if (needsReduction) {
        setDraftPhase("reduction");
        broadcastState(
          buildBroadcastPayload({ factions: cleaned, draftPhase: "reduction" }),
        );
      } else {
  ...
  const handleReduction = (playerIndex, category, componentIndex) => {
    ...
    const allPlayersReduced = fc.every((faction, idx) => {
      ...
      const playerComplete = categories.every((cat) => {
        ...
        console.log(
          `  ${cat}: ${currentCount}/${limit} ${isOver ? "❌ OVER" : "✓"}`,
        );
    ...
    if (allPlayersReduced) {
      ...
      console.log(
        "No swaps available - adding extra components and completing",
      );
  ...
  const handleRemoveComponentFromBuild = (
    playerIndex,
    category,
    componentIndex,
  ) => {
    ...
    if (
      !fc[playerIndex][category] ||
      !fc[playerIndex][category][componentIndex]
    ) {
      return;
    }
  ...
  const handleCompleteBuildPhase = () => {
    ...
    if (needsReduction) {
      ...
      if (allSwaps.length > 0) {
        console.log(
          `Found ${allSwaps.length} available swaps - moving to swap phase`,
        );
        setPendingSwaps(allSwaps);
        setDraftPhase("swap");
      } else {
  ...
  useEffect(() => {
    if (draftPhase === "swap" && pendingSwaps.length === 0) {
      console.log(
        "All swaps resolved - adding extra components and completing",
      );
      const factionsWithExtras = addAllExtraComponents(factions);
      setFactions(factionsWithExtras);
      setDraftPhase("complete");
  ...
  const renderCurrentPlayerInfo = () => {
      ...
      return (
        <div className="p-3 bg-orange-900/30 rounded-lg border border-orange-600">
          ...
          <div className="text-xs text-orange-300">
            Remove excess components to meet faction limits
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="p-3 bg-yellow-900/30 rounded-lg border border-yellow-600">
          <h3 className="font-bold text-yellow-400 text-sm">
            Configure settings and click "Start Draft" to begin
          </h3>
        </div>
      );
    }
    ...
    if (maxPicks === 0 && draftVariant !== "rotisserie") {
      ...
      return (
        <div className="p-3 bg-yellow-900/30 rounded-lg border border-yellow-600">
          <h3 className="font-bold text-yellow-400 text-sm">
            Player {currentPlayer + 1}'s Turn - Round {round}
          </h3>
          <div className="text-xs text-orange-400">
            Auto-passing: No available picks from this bag (all categories at
            limit)
          </div>
          <div className="text-xs text-gray-400">
            Bag {currentPlayer} of {playerBags.length} bags
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="p-3 bg-blue-900/30 rounded-lg border border-blue-600">
        ...
        <div className="text-xs text-blue-300">
          Pending picks: {pendingPicks.length} / {maxPicks} (must pick{" "}
          {maxPicks}){draftVariant === "rotisserie" && " (One pick per turn)"}
        </div>
        <div className="text-xs text-blue-300">Variant: {draftVariant}</div>
        <div className="text-xs text-gray-400">
          Bag {currentPlayer + 1} of {playerBags.length} bags
        </div>
            ...
            <div className="flex flex-wrap gap-1">
              {pendingPicks.map((pick, idx) => (
                <span
                  ...
                  onClick={() =>
                    handleRemovePendingPick(
                      pick.category,
                      pick.component.id || pick.component.name,
                    )
                  }
  ...
  return isMultiplayerGuest ? (
    <MultiplayerGuestView
      ...
      onSubmitPicks={(picks) =>
        sendToHost({ type: "PICK", playerIndex: myPlayerIndex, picks })
      }
      onSubmitFaction={(faction, phase) =>
        sendToHost({
          type: "FACTION_UPDATE",
            ...
            <div className="px-4 py-2">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end mobile-toolbar-actions">
                  ...
                  <button
                    className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold transition-colors mobile-action-button"
                ...
                {!draftStarted && (
                  <>
                    <div className="mb-3 p-4 bg-gray-900/70 rounded-lg border border-gray-700">
                      <h3 className="font-bold mb-4 text-yellow-400 text-sm">
                        Expansions
                      </h3>
                      ...
                      <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-200 ml-6">
                            Enables: 7 new factions, Agents, Commanders, Heroes,
                            Mechs, and new tiles.
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-200 ml-6">
                            Enables: 5 new factions, Breakthroughs, and new
                            tiles. (No Firmament/Obsidian)
                          </div>
                        </div>
                          ...
                          <label className="flex items-center cursor-pointer mb-1">
                            <input
                              ...
                              onChange={(e) =>
                                setExpansionsEnabled((prev) => ({
                                  ...prev,
                                  firmobs: e.target.checked,
                                }))
                              }
                              className="mr-2"
                            />
                            <span className="font-medium text-white text-sm">
                              Add Firmament/Obsidian
                            </span>
                          </label>
                          <div className="text-xs text-gray-200 ml-6">
                            Adds: The Firmament and The Obsidian to the draft.
                            (disabled by default)
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-200 ml-6">
                            Removes all base game factions and tiles, using only
                            Discordant Stars content. (Requires DS to be
                            enabled)
                          </div>
                        </div>
                  ...
                  <>
                    <div className="mb-4 p-4 bg-purple-900/30 rounded-lg border border-purple-600">
                      ...
                      <p className="text-purple-300 text-sm mb-2">
                        Build your factions from the components you drafted.
                        Click on components to add them to your build.
                      </p>
                      <p className="text-purple-300 text-sm mb-3">
                        You must stay within the standard faction limits. Your
                        drafted blue and red tiles are already included.
                      </p>
                      <button
                        onClick={
                          isMultiplayerHost
                            ? () => handleHostFactionSubmit("build")
                            : handleCompleteBuildPhase
                        }
                        ...
                        return (
                          <FrankenDrazBuilder
                            ...
                            onRemoveComponent={(category, index) =>
                              handleRemoveComponentFromBuild(
                                actualIndex,
                                category,
                                index,
                              )
                  ...
                  <>
                    <div className="mb-4 p-4 bg-orange-900/30 rounded-lg border border-orange-600">
                      ...
                      <p className="text-orange-300 text-sm">
                        Remove excess components from each faction to meet the
                        faction limits. Click any component to remove it.
                      </p>
                      {isMultiplayerHost && (
                        <button
                    ...
                    {visibleFactions.map((f, i) => {
                      ...
                      return (
                        <FactionSheet
                          ...
                          drafted={f}
                          onRemove={(cat, idx) =>
                            handleReduction(actualIndex, cat, idx)
                          }
                          onSwapComponent={(
                            playerIdx,
                            category,
                          ...
                          title={
                            isMultiplayerHost
                              ? "Your Faction — Remove Excess"
                              : `Player ${actualIndex + 1} - Remove Excess Components`
                          }
                  ...
                  <>
                    <div className="mb-4 p-4 bg-blue-900/30 rounded-lg border border-blue-600">
                      ...
                      <p className="text-blue-300 text-sm">
                        Review available component swaps. You can choose to swap
                        components or refuse the swap.
                      </p>
                      {isMultiplayerHost && (
                        <button
                    ...
                    {visibleFactions.map((f, i) => {
                      const actualIndex = isMultiplayerHost ? 0 : i;
                      const playerSwaps = pendingSwaps.filter(
                        (s) => s.playerIndex === actualIndex,
                      );
                      return (
                        <FactionSheet
                          key={actualIndex}
                          ...
                          title={
                            isMultiplayerHost
                              ? "Your Faction — Review Swaps"
                              : `Player ${actualIndex + 1} - Review Swaps`
                          }
                          showSwapHelper={true}
                    ...
                    <div className="mb-4 p-6 bg-gradient-to-r from-green-900/40 to-blue-900/40 rounded-xl border-2 border-green-500 shadow-xl">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <h3 className="font-bold text-green-400 text-2xl">
                          Draft Complete! Click 'Export Draft' before pressing
                          'Build Map' to save selections!
                        </h3>
                      </div>
                      <p className="text-green-300 mb-4">
                        All factions have been finalized with extra components
                        added. Review your custom factions below.
                      </p>
                      <div className="flex flex-wrap gap-3">
                        ...
                        <button
                          onClick={() => {
                            if (
                              confirm(
                                "Start a new draft? This will clear the current draft.",
                              )
                            ) {
                              cancelDraft();
                            }
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (isMultiplayerHost) {
                              ...
                              const factionsForMapBuilder = factions.map(
                                (faction, idx) => ({
                                  ...faction,
                              ...
                              Object.keys(multiplayer.peers).forEach(
                                (slotId) => {
                                  multiplayer.sendToPeer(slotId, {
                                    type: "NAVIGATE_MAP_BUILDER",
                                    data: {
                                      factions: factionsForMapBuilder,
                                      playerCount,
                                    },
                                  });
                                },
