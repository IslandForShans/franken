...
export default function SwapModal({
  ...
  const handleRefuse = () => {
    ...
    const newRefused = new Set(refusedNames).add(
      `${selectedSwap.name}|${selectedSwap.faction}`,
    );
  ...
  const renderComponentDetails = (component) => (
    <>
      ...
      {(component.combat !== undefined ||
        ...
        <div
          className="text-xs mt-2 grid grid-cols-2 gap-1"
          style={{ color: "#fff" }}
        >
          {component.cost !== undefined && (
            <div>
      ...
      {component.planets && component.planets.length > 0 && (
        <div className="mt-2 text-xs border-t border-gray-700 pt-2">
          ...
          {component.planets.map((planet, idx) => (
            <div
              key={idx}
              className="ml-2 mb-2 pb-2 border-b border-gray-700 last:border-b-0"
            >
              <div className="font-semibold" style={{ color: "#6ee7b7" }}>
                {planet.name}
              </div>
              <div className="grid grid-cols-2 gap-1 mt-1">
                <div>
                  <span className="font-semibold text-yellow-400">R:</span>{" "}
                  {planet.resource || 0}
                </div>
                <div>
                  <span className="font-semibold text-blue-400">I:</span>{" "}
                  {planet.influence || 0}
                </div>
              </div>
      ...
      {component.value !== undefined && (
        <div className="text-xs mt-2" style={{ color: "#fbbf24" }}>
          <span className="font-semibold">Commodity Value:</span>{" "}
          {component.value}
        </div>
      )}
    </>
      ...
      <div className="swap-modal-content">
        {step === "chooseSwap" && (
          <>
            ...
            >
              {swapOptions
                .filter((o) => !refusedNames.has(`${o.name}|${o.faction}`))
                .map((option, idx) => (
