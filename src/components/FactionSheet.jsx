...
export default function FactionSheet({
  ...
  const handleSwap = (category, index) => {
    const component = drafted[category][index];
    const availableSwaps = getSwapOptionsForTrigger(
      component.name,
      component.faction,
    );

    if (availableSwaps.length > 0) {
      // Enrich swap options with full component data
      const enrichedSwaps = availableSwaps.map((swap) => {
        const fullData = findFullComponentData(
          swap.name,
          swap.faction,
          swap.category,
        );
      ...
      if (filteredSwaps.length === 0) {
        alert(
          "No swap options available - all options are already in your faction.",
        );
        return;
      }
  ...
  const handleConfirmSwap = (swapOption, replaceIndex) => {
    ...
    onSwapComponent(
      ...
      replaceIndex,
      swapOption,
      swapModalData.triggerComponent,
    );

    setResolvedSwaps((prev) =>
      new Set(prev).add(getSwapKey(swapOption, swapModalData.triggerComponent)),
    );
  ...
  const handleRefuseSwap = (swapOption) => {
    if (onRefuseSwap && swapModalData) {
      onRefuseSwap(playerIndex, swapModalData.triggerComponent, swapOption);
    }
    setSwapModalOpen(false);
    setSwapModalData(null);
  };
          ...
          <div style={{ flex: 1 }}>
            ...
            {isTile && item.planets && item.planets.length > 0 && (
              <div className="text-xs mt-2" style={{ color: "#6ee7b7" }}>
                <span className="font-semibold">
                  {item.planets.length} Planet
                  {item.planets.length !== 1 ? "s" : ""}
                </span>
                {item.planets.map((p, idx) => (
                  ...
                  >
                    ...
                    {p.technology_specialty &&
                      p.technology_specialty.length > 0 && (
            ...
            {(category === "commanders" || category === "heroes") &&
              item.note && (
                  ...
                  <div style={{ marginLeft: "0.5rem" }}>
                    {item.techs.map((t, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          style={{ color: techColor, marginBottom: "0.25rem" }}
                        >
          ...
          <div className="faction-component-details">
            {extraComponents.length > 0 && (
              <div className="faction-component-extra-list">
                <div className="faction-component-extra-title">
                  EXTRA COMPONENTS:
                </div>
            ...
            {triggeredSwaps.length > 0 && (
              <div className="faction-component-swap-list">
                <div className="faction-component-swap-title">
                  SWAP OPTIONS:
                </div>
  ...
  return (
    <div
      className={`faction-sheet ${isCurrentPlayer ? "faction-sheet-current" : ""}`}
    >
      {/* Title */}
      <div className="card-header">
        <h2 className="faction-sheet-title">{title}</h2>
      </div>
      ...
      {showReductionHelper && Object.keys(draftLimits).length > 0 && (
        <div className="reduction-helper">
          ...
          <div className="reduction-helper-text">
            Remove excess components to meet faction limits. Red categories need
            reduction.
          </div>
        </div>
      )}
        ...
        {[1, 2, 3].map((col) => (
          ...
          >
            {getColumnCategories(col).map(({ key: category }) => {
              ...
              const availableSwaps = showReductionHelper
                ? getAvailableSwapsForCategory(category)
                : [];
              ...
              return (
                ...
                >
                  ...
                  <div className="faction-category-header">
                    ...
                    <div
                      className={`faction-category-count ${status.needsReduction ? "faction-category-count-over" : ""}`}
                    >
                  ...
                  {status.items.length === 0 ? (
                    ...
                    status.items.map((item, index) =>
                      renderComponent(item, category, index),
                    )
                  )}
                </div>
              );
      ...
      <SwapModal
        ...
        swapOptions={swapModalData?.swapOptions || []}
        triggerComponent={swapModalData?.triggerComponent}
        targetCategory={swapModalData?.swapOptions?.[0]?.category}
        availableReplacements={
          swapModalData?.swapOptions?.[0]?.category
